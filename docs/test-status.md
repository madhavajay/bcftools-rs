...
| `test_vcf_plugin` | partial-rust-covered | Static plugin registry/listing is in; `+counts`, `+missing2ref`, `+fill-AN-AC`, `+allele-length`, `+variant-distance`, `+check-ploidy`, `+tag2tag` (gl-to-pl/gp-to-gt), `+add-variantkey`, `+variantkey-hex`, `+remove-overlaps`, `+af-dist`, `+smpl-stats`, `+indel-stats`, and `+ad-bias` have Rust implementations (14 of 41). `+missing2ref`/`+fill-AN-AC`/`+allele-length`, all four `+variant-distance` modes, all three `+check-ploidy` fixtures, the two integer `+tag2tag` conversions, both VariantKey plugins (full `query.add-variantkey.vcf` / `variantkey-hex.out` fixtures), `+remove-overlaps` (all six `remove-overlaps.1.*` overlap/dup/`-O t`/`--reverse` fixtures), `+af-dist` (`af-dist.out`, HWE prob + AF-deviation histograms), `+smpl-stats` (`smpl-stats.1.out`, per-sample/per-site genotype stats), `+indel-stats` (`indel-stats.1.out`, SN/DVAF/DLEN/DFRAC/NFRAC), and `+ad-bias` (`ad-bias.out`, Fisher exact test on FMT/AD, both inputs) match their upstream `*.out` fixtures byte-for-byte. The umbrella Perl slice stays disabled until enough plugins land. |
