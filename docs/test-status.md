...
| `test_vcf_plugin` | partial-rust-covered | Static plugin registry/listing is in; `+counts`, `+missing2ref`, `+fill-AN-AC`, `+allele-length`, and `+variant-distance` have Rust implementations. `+missing2ref`/`+fill-AN-AC`/`+allele-length` and all four `+variant-distance` direction modes match their upstream `*.out` fixtures byte-for-byte. The umbrella Perl slice stays disabled until enough plugins land. |
