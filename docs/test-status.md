...
| `test_vcf_merge` | rust-covered | Rust integration coverage includes same-site sample-column concatenation, simple sorted text-mode exact-site union with missing FORMAT sample values, sites-only same-locus ALT union with AC/AN summing and byte-for-byte `merge.8.out` coverage, sampled plus sites-only same-locus ALT union with byte-for-byte `merge.9.1.out` coverage, byte-for-byte `merge.noidx.abc.out` coverage including `--no-index`, single-input `--force-single` byte-for-byte `merge.LPL.0.out` coverage, duplicate-sample handling, file-list input, output formats, and Kestrel reads; full allele unification, merge modes, INFO rules, gVCF, indexed regions, and upstream Perl parity remain. |
