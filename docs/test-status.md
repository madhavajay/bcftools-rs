...
| `test_vcf_plugin` | partial-rust-covered | Static plugin registry/listing is in; `+counts`, `+missing2ref`, `+fill-AN-AC`, `+allele-length`, `+variant-distance`, `+check-ploidy`, `+tag2tag` (gl-to-pl/gp-to-gt), `+add-variantkey`, `+variantkey-hex`, `+remove-overlaps`, `+af-dist`, `+smpl-stats`, `+indel-stats`, `+ad-bias`, `+prune`, `+dosage`, `+guess-ploidy`, `+contrast`, and `+fixref` have Rust implementations (19 of 41). `+missing2ref`/`+fill-AN-AC`/`+allele-length`, all four `+variant-distance` modes, all three `+check-ploidy` fixtures, the two integer `+tag2tag` conversions, both VariantKey plugins (full `query.add-variantkey.vcf` / `variantkey-hex.out` fixtures), `+remove-overlaps` (all six `remove-overlaps.1.*` overlap/dup/`-O t`/`--reverse` fixtures), `+af-dist` (`af-dist.out`, HWE prob + AF-deviation histograms), `+smpl-stats` (`smpl-stats.1.out`, per-sample/per-site genotype stats), `+indel-stats` (`indel-stats.1.out`, SN/DVAF/DLEN/DFRAC/NFRAC), `+ad-bias` (`ad-bias.out`, Fisher exact test on FMT/AD, both inputs), `+prune` (`prune.1.{1,2,3,4,6}.out` + `prune.2.1.out`, windowed maxAF/1st pruning **and** `calc_ld` r2/LD'/RD `-a`/`-m`/`-f` modes), `+dosage` (`dosage.{1,2,3}.out`, PL/GL/GT dosages), `+guess-ploidy` (`guess-ploidy.{PL,GL}.out`, sex inference from PL/GL likelihoods), `+contrast` (`contrast.out`/`.1.out`/`.1.1.out`/`.1.2.out`, control/case association + novel allele/genotype), and `+fixref` (`fixref.{4,5,6,7}.out`, FASTA REF/ALT ref-alt/swap/flip/flip-all) match their upstream `*.out` fixtures byte-for-byte. The umbrella Perl slice stays disabled until enough plugins land. |
