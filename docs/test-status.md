...
| `test_vcf_plugin` | partial-rust-covered | Static plugin registry/listing is in; `+counts`, `+missing2ref`, `+fill-AN-AC`, `+allele-length`, `+variant-distance`, and `+check-ploidy` have Rust implementations. `+missing2ref`/`+fill-AN-AC`/`+allele-length`, all four `+variant-distance` modes, and all three `+check-ploidy` fixtures (incl. `-m`) match their upstream `*.out` fixtures byte-for-byte. The umbrella Perl slice stays disabled until enough plugins land. |
