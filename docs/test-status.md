...
| `test_vcf_merge` | rust-covered | Rust integration coverage includes same-site sample-column concatenation, simple sorted text-mode exact-site union with missing FORMAT sample values, byte-for-byte `merge.noidx.abc.out` coverage including `--no-index`, duplicate-sample handling, file-list input, output formats, and Kestrel reads; full allele unification, merge modes, INFO rules, gVCF, indexed regions, and upstream Perl parity remain. |
