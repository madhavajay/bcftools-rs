...
| `test_vcf_head2` | blocked-local-tool | Rust integration coverage includes compressed VCF/BCF paths; Perl slice currently needs external `bgzip`/`tabix` availability in the harness. |
| `test_index` | rust-covered | Rust integration coverage covers CSI/TBI creation, stats, nrecords, stdin output paths, threads diagnostics, and large-coordinate CSI behavior; Perl slice not enabled yet. |
| `test_vcf_idxstats` | rust-covered | Rust integration coverage exists through `index -s/-n`; Perl slice not enabled yet. |
...
| `test_tabix` | blocked-local-tool | Rust integration coverage includes BGZF indexing/querying for common presets; Perl slice requires external `bgzip` on `PATH`. |
