...
fn merge_abc_missing_to_ref_fixture_matches_upstream_text_output() {
    ...
    assert_eq!(
        code, 0,
        "merge.abc --force-samples -0 fixture failed: {err}"
    );

    let expected = std::fs::read_to_string("../../bcftools/test/merge.abc.3.out").unwrap();
    assert_eq!(out, expected);
}
...
fn gvcf_merge_reference_block_fixture_matches_upstream_text_output() {
    let (out, err, code) = run(&[
        ...
        "--gvcf",
        "-",
        "../../bcftools/test/gvcf.merge.1.vcf",
        "../../bcftools/test/gvcf.merge.2.vcf",
        "../../bcftools/test/gvcf.merge.3.vcf",
    ]);
    assert_eq!(code, 0, "gvcf.merge.1 fixture failed: {err}");

    let expected = std::fs::read_to_string("../../bcftools/test/gvcf.merge.1.out").unwrap();
    assert_eq!(out, expected);
}

#[test]
fn merge_gvcf7_single_base_end_fixture_matches_upstream_text_output() {
    let (out, err, code) = run(&[
        "merge",
        "--no-version",
        "--gvcf",
        "-",
        "../../bcftools/test/merge.gvcf.7.a.vcf",
        "../../bcftools/test/merge.gvcf.7.b.vcf",
    ]);
    assert_eq!(code, 0, "merge.gvcf.7 fixture failed: {err}");

    let expected = std::fs::read_to_string("../../bcftools/test/merge.gvcf.7.out").unwrap();
    assert_eq!(out, expected);
}

#[test]
fn merge_gvcf8_no_reference_block_split_fixture_matches_upstream_text_output() {
    let (out, err, code) = run(&[
        "merge",
        "--no-version",
        "--gvcf",
        "-",
        "../../bcftools/test/merge.gvcf.8.a.vcf",
        "../../bcftools/test/merge.gvcf.8.b.vcf",
    ]);
    assert_eq!(code, 0, "merge.gvcf.8 fixture failed: {err}");

    let expected = std::fs::read_to_string("../../bcftools/test/merge.gvcf.8.out").unwrap();
    assert_eq!(out, expected);
}
...
#[test]
fn merge_mrules_missing_rules_fixture_matches_upstream_text_output() {
    let (out, err, code) = run(&[
        "merge",
        "--no-version",
        "--gvcf",
    ...
    assert_eq!(code, 0, "merge.mrules.1.2 fixture failed: {err}");

    let expected = std::fs::read_to_string("../../bcftools/test/merge.mrules.1.2.out").unwrap();
    assert_eq!(out, expected);
}
...
fn merge_gvcf5_non_ref_order_fixture_matches_upstream_text_output() {
    for extra_args in [Vec::<&str>::new(), vec!["--merge", "none"]] {
        let mut args = vec![
            ...
            "--gvcf",
            "-",
            "../../bcftools/test/merge.gvcf.5.a.vcf",
            "../../bcftools/test/merge.gvcf.5.b.vcf",
        ];
        args.splice(4..4, extra_args);
        let (out, err, code) = run(&args);
        assert_eq!(code, 0, "merge.gvcf.5 fixture failed for {args:?}: {err}");

        let expected = std::fs::read_to_string("../../bcftools/test/merge.gvcf.5.1.out").unwrap();
        assert_eq!(out, expected, "merge.gvcf.5 fixture differed for {args:?}");
    }
}

#[test]
fn merge_gvcf11_existing_fixture_matches_upstream_text_output() {
    let (out, err, code) = run(&[
        "merge",
        "--no-version",
        "--gvcf",
        "-",
        "../../bcftools/test/merge.gvcf.11.a.vcf",
        "../../bcftools/test/merge.gvcf.11.b.vcf",
        "../../bcftools/test/merge.gvcf.11.c.vcf",
    ]);
    assert_eq!(code, 0, "merge.gvcf.11 fixture failed: {err}");

    let expected = std::fs::read_to_string("../../bcftools/test/merge.gvcf.11.1.out").unwrap();
    assert_eq!(out, expected);
}
