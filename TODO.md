...
- [ ] `query` (`vcfquery.c`, 20k) — `-f` format-string output, `-l/--list-samples`, region/target restriction, `--include`/`--exclude`. Depends on Phase 1 `convert` formatter + filter engine. Covered by `test_vcf_query`.
  - [x] Snapshot coverage: `-l/--list-samples` for VCF/VCF.gz/BCF, `-s`/`-S` sample selection including `^` exclusion, and a small text-backed `-f` formatter for core fields, INFO lookups, `%SAMPLE`, and simple FORMAT/sample loops.
  - [ ] Remaining: full `convert.c` formatter grammar, functions, GT special forms, regions/targets, `-H` headers, and `-i`/`-e` filtering.
