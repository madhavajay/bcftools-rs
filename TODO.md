...
- [ ] `query` (`vcfquery.c`, 20k) — `-f` format-string output, `-l/--list-samples`, region/target restriction, `--include`/`--exclude`. Depends on Phase 1 `convert` formatter + filter engine. Covered by `test_vcf_query`.
  - [x] Snapshot coverage: `-l/--list-samples` for VCF/VCF.gz/BCF, `-s`/`-S` sample selection including `^` exclusion, `-H`/`-HH` column headers for simple formats, POS-based `-r`/`-R`/`-t`/`-T` region and target restriction including braced contig names, limited record-level `-i`/`-e` filtering for core/INFO expressions including missing INFO values, comma-separated string element matches, `@file` string membership, `strlen(...)`, indexed `AC`, and computed `AF`/`MAC`/`MAF`, simple ALT/INFO vector predicates (`ALT[*]~"..."`, `ALT="*"`, `TAG[*]="."`, `TAG[*]!="."`), `FILTER` ID comparisons, simple FORMAT/sample predicates (`GT="."`, `GT="0|1"`, `GT="hom"`, and `FMT/`/`FORMAT/` tags), simple sample-count filters (`N_PASS(...)`, `COUNT(...)`, `smpl_count(...)`), modulo comparisons, simple computed fields (`N_ALT`, `N_SAMPLES`, exact/regex/negated `TYPE`, `%ILEN`), and native bcftools-rs fallback expression evaluation for text records including simple `phred(binom(FMT/AD))` and `phred(fisher(INFO/DP4))`; small text-backed `-f` formatter for core fields, implicit record newlines, `%LINE`, `%FORMAT`, INFO lookups, `%SAMPLE`, forced record namespace `%/TAG`, `%N_PASS(...)` sample counts, simple FORMAT/sample loops, upstream-backed `query.func.1` numeric formatter fixtures, `%smpl_count(FMT/TAG)` sample-loop formatting, limited numeric functions (`SUM`/`AVG`/`MEAN`/`MIN`/`MAX`/`ABS`) over INFO and FORMAT values, and limited sample-loop `%PBINOM(TAG)`.
